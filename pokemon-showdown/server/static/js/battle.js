...
};this.id=options.id||'';var hasFrame=!!(options.$frame&&options.$frame.length);var hasLog=!!(options.$logFrame&&options.$logFrame.length);if(hasFrame&&hasLog){this.scene=new BattleScene(this,options.$frame,options.$logFrame);}else if(!options.$frame&&!options.$logFrame){this.scene=new BattleSceneStub();}else{console.warn("Battle: frame or log missing. options:",options);var $fr=(options.$frame&&options.$frame.length)?options.$frame:$('.battle');var $log=(options.$logFrame&&options.$logFrame.length)?options.$logFrame:$('.battle-log');if($fr.length&&$log.length){this.scene=new BattleScene(this,$fr,$log);}else{this.scene=new BattleSceneStub();}}this.paused=!!options.paused;this.started=!this.paused;this.debug=!!options.debug;if(typeof options.log==='string')options.log=options.log.split('\n');this.stepQueue=options.log||[];this.subscription=options.subscription||null;this.autoresize=!!options.autoresize;this.p1=new Side(this,0);this.p2=new Side(this,1);this.sides=[this.p1,this.p2];this.p2.foe=this.p1;this.p1.foe=this.p2;this.nearSide=this.mySide=this.p1;this.farSide=this.p2;this.resetStep();if(this.autoresize){window.addEventListener('resize',this.onResize);this.onResize();}}var _proto3=Battle.prototype;_proto3.
