...
var notify=(PS.prefs.tournaments||'notify')==='notify'||
...
preact.h("select",{onChange:this.selectChallengeUser,value:tour.selectedChallenge,"class":"select"},
info.challenges.map(function(challenge,index){return(
preact.h("option",{value:index},challenge));}
)
)
)
),
...
};return TourPopOutPanel;}(PSRoomPanel);TourPopOutPanel.id='tourpopout';TourPopOutPanel.routes=['tourpopout'];TourPopOutPanel.location='modal-popup';TourPopOutPanel.noURL=true;
